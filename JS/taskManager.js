...
const createTaskHtml = (myName, floatingTextarea2, assign, status1, date, id ) => {
    ...
    <div class="mt-5 me-2 card ${status1 === "Done"? "border-success text-success" : status1 === "In Progress" ? "border-warning text-warning": status1 === "Review" ? "border-danger text-danger": "border-info text-info"}" style="width: 25rem;" data-task-id="${id}" >
      <div class=" h-100">
      ...
      <div class="card-body">
        ...
        <p class="card-text"><strong>Status :</strong>  <span> ${status1}</span></p>
...
class TaskManager {
    ...
    console.log(currentTask.status1);
    console.log(typeof currentTask.status1);
    if(currentTask.status1 === "To Do"){
        console.log("inside to do");
    const html = document.querySelector('#to_do');
    html.innerHTML = newTaskhtml;
    } else if(currentTask.status1 === "Pending"){
        const  html= document.querySelector('#pending');
        html.innerHTML = newTaskhtml;
    } else if(currentTask.status1 === "Done"){
        const  html= document.querySelector('#done');
        html.innerHTML = newTaskhtml;
    } else if (currentTask.status1 === "In Progress"){
        const  html= document.querySelector('#progress');
        html.innerHTML = newTaskhtml;
    }
